...
class TestConvertDates:
    def test_converts_date_to_string(self):
        """Verify date objects are converted to ISO strings."""
    ...
    def test_handles_nested_structures(self):
        """Verify nested dicts and lists have dates converted recursively."""
        data = {
            "tasks": [
    ...
    def test_passes_through_normal_values(self):
        """Verify non-date values pass through unchanged."""
...
class TestServerConfig:
    def test_defaults(self):
        """Verify ServerConfig has correct default values."""
        config = ServerConfig()
...
class TestLoadConfig:
    def test_loads_from_yaml(self, tmp_path):
        """Verify config is loaded correctly from a YAML file."""
        config_data = {
            "server": {"port": 9000},
            "repos": [
                {
    ...
    def test_returns_default_for_missing_file(self, tmp_path):
        """Verify missing config file returns default AppConfig."""
        config = load_config(tmp_path / "nonexistent.yaml")
        assert isinstance(config, AppConfig)
        assert len(config.repos) == 0

    def test_returns_default_for_empty_file(self, tmp_path):
        """Verify empty config file returns default AppConfig."""
        config_file = tmp_path / "config.yaml"
    ...
    def test_handles_dates_in_yaml(self, tmp_path):
        """Verify YAML dates are handled without errors during config load."""
        config_file = tmp_path / "config.yaml"
